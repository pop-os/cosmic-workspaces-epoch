...
impl ToplevelLayout for RowColToplevelLayout {
    ...
    fn layout(
        ...
        if requested_main_total > self.axis.main(max_limit) && toplevels.len() > 1 {
            let max_requested_cross = toplevels
                .iter()
                .map(|t| self.axis.cross(t.preferred_size))
                .reduce(f32::max)
                .unwrap();
            let cross_scale_factor = (self.axis.cross(max_limit) / 2.) / max_requested_cross;
            ...
            let (split_point, two_row_scale_factor) = (1..toplevels.len())
                .map(|i| {
                    ...
                    let top_total = self.requested_main_total(top_row);
                    let bottom_total = self.requested_main_total(bottom_row);
                    let max = top_total.max(bottom_total);
                    let scale_factor = (self.axis.main(max_limit) / max).min(1.0);
                    (i, scale_factor)
                })
            ...
            if two_row_scale_factor > scale_factor {
                ...
                return toplevels
                    ...
                    .map(move |(i, child)| {
                        ...
                        let max_main = requested_main * two_row_scale_factor;

                        let (width, height) = self.axis.pack(max_main, max_cross / 2.);
                        let (x, y) = self.axis.pack(total_main, cross_pos);
                        total_main += max_main;
